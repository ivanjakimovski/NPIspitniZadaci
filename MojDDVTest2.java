...
class MojDDV {
    ...
    public void printStatistics (OutputStream outputStream) {
        ...
        float min=Float.MAX_VALUE, max=Float.MIN_VALUE, sum=0, count=0, avg = 0;

        for(int i=0; i<smetki.size(); i++) {
            if(smetki.get(i).getTaxReturn()<min) {
                min = smetki.get(i).getTaxReturn();
            }
            if(smetki.get(i).getTaxReturn()>max) {
                max = smetki.get(i).getTaxReturn();
            }
            sum+=smetki.get(i).getTaxReturn();
